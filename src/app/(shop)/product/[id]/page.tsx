    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Breadcrumb
        items={[
          ...
          {
            label: brandName,
            href: `/category/${categorySlug}?brand=${brandName?.toLowerCase()}`,
          },
          { label: product.name },
        ]}
      />
      ...
      <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 mt-6">
        ...
        >
          <div>
            <span className="text-sm text-accent font-body uppercase tracking-wider">
              {brandName}
            </span>
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-heading font-bold mt-1">
              {product.name}
            </h1>
          </div>
            ...
            <div className="flex items-center gap-1">
              {[1, 2, 3, 4, 5].map((star) => (
                <svg
                  ...
                  viewBox="0 0 24 24"
                  fill={star <= Math.floor(productRating) ? "currentColor" : "none"}
                  stroke="currentColor"
                  className={`w-4 h-4 ${
                    star <= Math.floor(productRating) ? "text-warning" : "text-text-primary/20"
                  }`}
                >
                  <path
                    strokeLinecap="round"
          ...
          {colors.length > 0 && (
            <div>
              <h4 className="text-sm font-heading font-semibold mb-3">
                Color: <span className="text-text-primary/60">{selectedColor}</span>
              </h4>
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-md text-sm border transition-all ${
                        selectedColor === colorName
                          ...
                          : "border-secondary-light text-text-primary/60 hover:border-text-primary/30"
                      }`}
                    >
                      {colorName}
                    </button>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button
              ...
              className={`btn-primary flex-1 text-base ${addedToCart ? "bg-success hover:bg-success" : ""}`}
            >
              {addedToCart
                ? "✓ Added!"
            ...
            <button
              ...
              className={`btn-secondary p-3 ${inWishlist ? "text-error border-error" : ""}`}
          ...
          <div className="card p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium">Pay with Cryptocurrency</p>
              <p className="text-xs text-text-primary/50">
                BTC, ETH, USDT & 100+ more accepted
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3 pt-4 border-t border-secondary-light">
            <div className="flex items-center gap-3 text-sm">
              ...
              <span className="text-text-primary/60">Free shipping on orders over $500</span>
            </div>
            <div className="flex items-center gap-3 text-sm">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-success">
                <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182M16.5 19.5a2.25 2.25 0 0 0 2.25-2.25v-1.5M7.5 4.5a2.25 2.25 0 0 1 2.25 2.25v1.5" />
              </svg>
              <span className="text-text-primary/60">30-day easy returns</span>
            </div>
            <div className="flex items-center gap-3 text-sm">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-success">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" />
              </svg>
              <span className="text-text-primary/60">2-year warranty included</span>
            </div>
          </div>
        </motion.div>
      </div>
      ...
      <div className="mt-16">
          ...
          <div className="flex gap-8">
            {(["description", "specs", "reviews"] as const).map((tab) => (
              <button
                ...
                className={`pb-4 text-sm font-heading font-semibold capitalize border-b-2 transition-colors ${
                  activeTab === tab
        ...
        <div className="py-8">
          {activeTab === "description" && (
            ...
            >
              <p className="text-text-primary/70 leading-relaxed whitespace-pre-line">
                {product.description}
              </p>
            </motion.div>
          )}
                  ...
                  <table className="w-full">
                    <tbody>
                      {specs.map((spec: any, index: number) => (
                        <tr
                          key={index}
                          className={index % 2 === 0 ? "bg-secondary-dark/50" : ""}
                        >
                          <td className="px-6 py-3 text-sm font-medium text-text-primary w-1/3">
                            {spec.label}
                          </td>
                          <td className="px-6 py-3 text-sm text-text-primary/70">
                            {spec.value}
                          </td>
                        </tr>
                      ))}
