      ...
      .then((d) => {
        ...
        if (found) {
          ...
          const schema = {
            ...
            mainEntityOfPage: `${SITE_URL}/blog/${found.slug || found.id}`,
          };
  ...
  const renderContent = (content: string): React.ReactNode => {
    ...
    return content
      ...
      .map((line, i) => {
        if (line.startsWith("## "))
          return (
            <h2 key={i} className="text-xl sm:text-2xl font-heading font-bold mt-8 mb-4">
              {line.replace("## ", "")}
            </h2>
          );
        if (line.startsWith("### "))
          return (
            <h3 key={i} className="text-lg sm:text-xl font-heading font-semibold mt-6 mb-3">
              {line.replace("### ", "")}
            </h3>
          );
        if (line.startsWith("- "))
          return (
            <li key={i} className="ml-6 mb-2 text-text-primary/70 list-disc">
              {renderLinks(line.replace("- ", ""))}
            </li>
          );
        if (line.match(/^\d\./))
          return (
            <li key={i} className="ml-6 mb-2 text-text-primary/70 list-decimal">
              {renderLinks(line.replace(/^\d\. /, ""))}
            </li>
          );
        return (
          <p key={i} className="text-text-primary/70 leading-relaxed mb-3">
            {renderLinks(line)}
          </p>
        );
      });
