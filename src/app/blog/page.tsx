  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-12">
        {categories.map((cat) => (
          <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 py-2 rounded-full text-sm font-body transition-colors ${activeCategory === cat ? "bg-accent text-white" : "bg-secondary text-text-primary/60 hover:text-text-primary hover:bg-secondary-light"}`}>{cat}</button>
        ))}
      </div>

      {loading ? (
        <p className="text-center text-text-primary/50">Loading articles...</p>
      ) : filteredPosts.length === 0 ? (
        <div className="text-center py-16">
          <p className="text-text-primary/50">No articles yet. Check back soon!</p>
        </div>
      ) : (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-full bg-accent/20 flex items-center justify-center">
                      <span className="text-xs font-heading font-bold text-accent">{post.author?.charAt(0) || "C"}</span>
                    </div>
                    <span className="text-xs text-text-primary/40">{post.author || "Cryptoelectro Team"}</span>
                  </div>
